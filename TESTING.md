...
Per-target regression checks cover pending versus affected summaries, no-resistance starts, old-record review, GM permissions, original-target binding, stale clicks, independent ending, unrelated conditions, shared condition markers, expiry cleanup, and recoverable marker-write failures. Test mixed resistance outcomes on two unlinked targets, then end one and let the other expire.
