...
export function recoveryCommandFromSpell(spell) {
  ...
  const likelyFields = [
    "vtt_notes", "vttNotes", "notes", "note", "otf", "formula",
    "effect", "effects", "description", "text", "usage", "usage_notes"
  ];
...
export function parseRecoveryExpression(rawValue, fallbackType = "none") {
  const original = String(rawValue ?? "").replace(/[–—−]/g, "-").trim();
  ...
  if (diceMatch) {
    ...
    if (!Number.isFinite(dice) || dice <= 0 || dice > 1000 || !Number.isFinite(faces) || faces <= 0 || faces > 1000000) {
...
export function sentenceCaseDamageType(text) {
  ...
  for(const [rx,type] of [[/huge piercing|pi\+\+/,'pi++'],[/large piercing|pi\+/,'pi+'],[/small piercing|pi-/,'pi-'],[/piercing|\bpi\b/,'pi'],[/burn|fire|flame|heat|electric|lightning/,'burn'],[/crush|impact|force|concussion/,'cr'],[/cut|slash/,'cut'],[/impal|\bimp\b/,'imp'],[/corrosion|acid|\bcor\b/,'cor'],[/toxic|poison|\btox\b/,'tox'],[/fatigue|\bfat\b/,'fat']])if(rx.test(raw))return type;
  return '';
}
export function normaliseDamageFormula(formula, context='') {
  const text=String(formula||'').replace(/[–—−]/g,'-').trim();
  const match=/(\d+)d(?:6)?([+-]\d+)?(!?)/i.exec(text);if(!match)return '';
  const after=text.slice(match.index+match[0].length);
  const explicit=/(?:^|\s)(pi\+\+|pi\+|pi-|pi|burn|cr|cut|imp|cor|tox|fat)(?=\s|$)/i.exec(after);
  const type=explicit?.[1]||sentenceCaseDamageType(after+' '+context);if(!type)return '';
  const divisor=/\(\s*(\d+(?:\.\d+)?)\s*\)/.exec(after)?.[0]||'';
  const extras=[/\bex\b|explosive/i.test(after+' '+context)?'ex':'',/surge/i.test(after+' '+context)?'surge':''];
  return [`${match[1]}d${match[2]||''}${match[3]}`,divisor,type,...extras].filter(Boolean).join(' ');
}
...
export function parseProfileText(rawText) {
  ...
  const firstPlainLine = lines.find(line => !/^\s*(spell effects|inherent modifiers|typical casting|cost|energy|damage|effect|name|title)\s*:/i.test(line));
  ...
  let damageFormula = "";
  const existingDamage = text.match(/\b(\+?\d+d(?:6)?(?:[+-]\d+)?\s*(?:burn|cr|cut|imp|pi\+\+|pi\+|pi-|pi|cor|tox|fat)(?:\s+ex|\s+surge)?)\b/i);
  if (existingDamage) {
  ...
  if (!damageFormula) {
    ...
    if (damageLine) {
      damageFormula = normaliseDamageFormula(`${damageLine[1]} ${damageLine[2] ?? ""}`, `${spellEffects} ${inherentModifiers} ${typicalCasting}`);
    }
  }
  ...
  if (explicitRecoveryCommand) {
    ...
    if (/\b(restore fp|recover fp|recover fatigue|restore fatigue|lend energy|fatigue recovery)\b/.test(lowered)) {
      effectType = "restore-fp";
    }

    const recoveryLine = lines.find(line => /\b(heal|healing|restore hp|restore fp|recover fp|recover fatigue|lend energy|lend vitality|fatigue recovery)\b/i.test(line));
