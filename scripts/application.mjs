...
export class CastingWindow extends foundry.applications.api.ApplicationV2 {
  async renderQuiet() {
    ...
    try {
      return await this.render({ force: false });
    } finally {
      this._quietRenders--;
    }
  }
  bringToFront(...args) {
    if (!this._quietRenders) return super.bringToFront(...args);
  }
  raiseWindow() {
...
export async function openCharacterSheet(actor) {
  ...
  if (!sheet) throw new Error('This character has no available sheet.');
