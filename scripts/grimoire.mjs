...
const button = (action, label, attrs = '') =>
  `<button type="button" data-grimoire="${action}" ${attrs}>${label}</button>`;
const option = (v, l, selected) =>
    ...
    buttons: [
      {
        ...
        default: true,
        callback: (_event, b, d) =>
          new FormData(b.form || d.form || d.element.querySelector('form')),
      },
      { action: 'cancel', label: 'Cancel', callback: () => null },
    ],
  });
  return result instanceof FormData
    ? { name: String(result.get('name') || '').trim(), tags: cleanTags(result.get('tags')) }
    : null;
}

export class Grimoire extends App {
  static DEFAULT_OPTIONS = {
    id: 'gga-casting-grimoire',
    tag: 'section',
    classes: ['gca-window', 'gca-grimoire-window'],
    window: { title: 'Grimoire', icon: 'fa-solid fa-book-open', resizable: true },
  ...
  get filters() {
    return { ...this.prefs.views[this.prefs.tab], tab: this.prefs.tab };
  }
  items() {
    return catalogue(this.actor, bookmarksFor(this.actor));
  }
  async persist() {
  ...
  async selectActor(actor) {
    if (this.busy) throw new Error('Finish the current build operation before changing character.');
    own(actor);
    this.actor = actor;
    this.selected = null;
    this.status = 'Select an entry to explore it.';
  ...
  refreshActor(actor) {
    ...
    this._refreshTimer = setTimeout(() => {
      if (this.rendered && !this.busy) this.renderQuiet();
    }, 100);
  }
  error(error) {
  ...
  card(item) {
    const profile = item.type === 'profile',
      colour = profile
        ? 'build'
    ...
    return `<article class="gca-book-card ${item.id === this.selected ? 'is-selected' : ''}" data-school="${esc(colour)}">
      ...
      ${button(
        ...
        <span class="gca-book-card-bottom">${
          profile
            ? item.tags
                .slice(0, 3)
                .map((t) => `<span class="gca-book-tag">${esc(t)}</span>`)
                .join('')
            : ''
        }${item.issues.length ? '<span class="gca-book-review">Review setup</span>' : ''}</span>`,
  ...
  details(item) {
    if (!item)
      return `<div class="gca-book-no-selection"><span aria-hidden="true">◇</span><h3>Your next spell starts here</h3><p>Select a card or row to see its details and prepare a casting setup.</p></div>`;
    const p = item.profile,
      details = p
        ? [
            ['Skill', numberLabel(item.level)],
            ['Base energy', item.cost],
            ['Type', item.fullClass],
            ...(item.colleges.length ? [['College', item.colleges.join(', ')]] : []),
            ...(item.castTime !== '—' ? [['Casting time', item.castTime]] : []),
            ...(item.duration !== '—' ? [['Duration', item.duration]] : []),
          ]
        : [
            ['Skill', numberLabel(item.level)],
            ['Class', item.fullClass || '—'],
            ['College', item.colleges.join(', ') || '—'],
            ['Cost', item.cost],
            ['Maintenance', item.maintain],
            ['Casting time', item.castTime],
            ['Duration', item.duration],
          ];
    const active = p ? activeProfile(p) : null,
      effect =
        active?.effectType === 'heal-hp'
          ? 'HP healing'
          : active?.effectType === 'restore-fp'
            ? 'FP recovery'
            : 'None';
    ...
    return `<header class="gca-book-detail-top"><div class="gca-kicker">${esc(p ? 'SAVED BUILD' : item.kind === 'spell' ? 'SPELL' : 'SKILL / POWER')}</div><h2>${esc(item.name)}</h2><p>${esc(p ? RULE_LABELS[p.rules] || p.rules : item.colleges.join(' · ') || 'Character skill')}</p>
      ...
      ${
        parsed
          ? `<section class="gca-book-detail-section"><h3>Parsed build</h3><dl>${[
              ['Detected energy', parsed.energy ?? 'Not found'],
              ['Spell effects', plainText(parsed.spellEffects) || '—'],
              ...
              .join(
                '',
              )}</dl><p class="gca-hint">The saved casting setup above includes any changes made after parsing.</p></section>`
          : ''
      }
  ...
  async _renderHTML() {
    ...
    root.className = 'gca-root gca-book-root';
    const all = this.items(),
      filters = this.filters,
      visible = sortItems(filterItems(all, filters), filters.sort),
      facet = facets(all, this.prefs.tab);
    if (!visible.some((i) => i.id === this.selected)) this.selected = visible[0]?.id || null;
    const selected = visible.find((i) => i.id === this.selected),
      builds = this.prefs.tab === 'builds';
    const counts = {
      spells: all.filter((i) => i.type === 'ability' && i.kind === 'spell').length,
      skills: all.filter((i) => i.type === 'ability' && i.kind === 'skill').length,
      builds: all.filter((i) => i.type === 'profile').length,
    };
    const actors = [
      ...new Map(
        [
    ...
    root.innerHTML = `<header class="gca-book-header"><div><div class="gca-kicker">GURPS 4e · ${esc(this.actor.name)}</div><h1>Grimoire</h1><p>Spells, powers, and the builds you return to.</p></div><div class="gca-book-header-tools"><label class="gca-sr" for="gca-book-actor">Character</label><select id="gca-book-actor" data-book-actor>${actors.map((a) => option(a.uuid, a.name, this.actor.uuid)).join('')}</select>${button('casting', 'Casting assistant')}${button('new-build', '+ New RPM build', 'class="gca-primary"')}</div></header>
      <div class="gca-book-navigation"><nav class="gca-tabs" aria-label="Grimoire sections">${[
        ...
        .map(([t, label]) =>
          button(
            'tab',
            `${label} <span>${counts[t]}</span>`,
            `data-tab="${t}" aria-pressed="${this.prefs.tab === t}"`,
          ),
        )
        .join(
          '',
  ...
  _replaceHTML(root, content) {
    const doc = content.ownerDocument,
      active = content.contains(doc.activeElement) ? doc.activeElement : null,
      field = active?.dataset.browse,
    ...
    if (field) {
      const el = root.querySelector(`[data-browse="${field}"]`);
      el?.focus();
      if (el?.type === 'search' && start != null) el.setSelectionRange(start, end);
    } else if (focusAction) {
    ...
    root.addEventListener('input', (event) => {
      if (event.target.dataset.browse === 'query') this.change(event).catch((e) => this.error(e));
    });
    root.addEventListener('change', (event) => {
      if (event.target.tagName === 'SELECT' || event.target.hasAttribute('data-book-import'))
  ...
  async change(event) {
    ...
    if (el.dataset.browse) {
      this.prefs.views[this.prefs.tab][el.dataset.browse] = el.value;
      await this.renderQuiet();
      await this.persist();
      return;
    }
    if (el.hasAttribute('data-book-actor')) {
      const actor = await fromUuid(el.value);
      await this.selectActor(actor);
      return;
    }
    if (el.hasAttribute('data-book-import') && el.files[0]) {
      ...
      await this.work(async () => {
        ...
        this.prefs.tab = 'builds';
        this.prefs.views.builds = viewDefaults();
        this.selected = imported[0] ? `profile:${imported[0].id}` : null;
        this.status = `Imported ${imported.length} builds.  Review their casting references before use.`;
      });
    }
  }
  async work(task) {
    ...
    try {
      ...
      this.busy = false;
      if (this.rendered) await this.renderQuiet();
    }
  }
  async click(event) {
    const el = event.target.closest('[data-grimoire],[data-page-ref]');
    ...
    const all = this.items(),
      item = all.find((i) => i.id === (el.dataset.id || this.selected)),
      action = el.dataset.grimoire;
    const currentProfile = () => {
      const p = profiles(this.actor).find((p) => p.id === item?.profile?.id);
      if (!p) throw new Error('This build is no longer saved.');
      return p;
    };
    switch (action) {
      case 'select':
      ...
      case 'clear':
        ...
        break;
      case 'favourites':
        this.prefs.views[this.prefs.tab].favourites = !this.filters.favourites;
        break;
      case 'tag':
        this.prefs.views.builds.tag = el.dataset.value;
        break;
      case 'favourite': {
        if (!item) return;
        const allMarks = clone(game.user.getFlag(ID, 'grimoireFavourites') || []);
        const index = allMarks.findIndex(
          (m) => m.actorUuid === this.actor.uuid && bookmarkMatches(m, item, all),
        );
        if (index >= 0) allMarks.splice(index, 1);
        else allMarks.push({ actorUuid: this.actor.uuid, ...bookmarkFor(item) });
        await this.work(() => game.user.setFlag(ID, 'grimoireFavourites', allMarks));
        break;
      }
      case 'casting':
        event.stopPropagation();
        await this.actions.open(this.actor.uuid);
        return;
      case 'prepare':
        event.stopPropagation();
        if (!item) return;
        await (item.type === 'profile'
          ? this.actions.open(this.actor.uuid, item.profile.id)
          : this.actions.openAbility(this.actor.uuid, reference(item.entry)));
        return;
      case 'open-profile':
      ...
      case 'new-build':
        event.stopPropagation();
        await this.actions.newBuild(this.actor.uuid);
        return;
      case 'sheet':
        event.stopPropagation();
      ...
      case 'metadata':
        await this.work(async () => {
          const before = currentProfile(),
            values = await metadataDialog(before);
          if (!values) return;
          await saveProfile(this.actor, { ...currentProfile(), ...values });
          this.status = 'Build name and tags saved.';
        });
        break;
      case 'duplicate':
        await this.work(async () => {
          const p = currentProfile(),
            copy = await saveProfile(
              this.actor,
              { ...p, name: `${p.name.slice(0, 113)} (copy)` },
              { copy: true },
            );
          this.selected = `profile:${copy.id}`;
          this.prefs.views.builds = viewDefaults();
          this.status = 'Build duplicated.  Use Name & tags to rename it.';
        });
        break;
      case 'delete':
        await this.work(async () => {
          ...
          if (
            !(await confirm(
              'Delete saved build',
              `<p>Delete <strong>${esc(p.name)}</strong> from ${esc(this.actor.name)}?</p>`,
            ))
          )
            return;
          await deleteProfile(this.actor, p.id);
          this.selected = null;
          this.status = 'Saved build deleted.';
        });
        break;
      case 'shortcut':
        await shortcut(this.actor, currentProfile());
        return;
      case 'export':
        download(exportProfiles(this.actor, [currentProfile()]), 'grimoire-build.json');
        return;
      case 'export-view': {
        const visible = sortItems(filterItems(all, this.filters), this.filters.sort).filter(
          (i) => i.type === 'profile',
        );
        ...
        download(
          exportProfiles(
            this.actor,
            visible.map((i) => i.profile),
          ),
          'grimoire-builds.json',
        );
        return;
      }
      case 'import':
        this.element.querySelector('[data-book-import]').click();
        return;
    }
    await this.renderQuiet();
    await this.persist();
